<!-- src/components/accounts/AccountPicker.vue -->
...
const { currentBalanceById, refreshAllCurrentBalances } = useAccounts();
...
const selectedBalance = computed(() => {
    ...
    if (!id) return "";
    const v = currentBalanceById.value.get(id) ?? 0;
    return formatCentsEUR(v);
});

const totalAll = computed(() => {
    return props.accounts.reduce((sum, a) => sum + (currentBalanceById.value.get(a.id) ?? 0), 0);
});
...
onMounted(() => {
    ...
    // ensure balances are present when the picker opens (first page load)
...
<template>
    <div v-click-outside="() => (open = false)" class="relative">
        <Button variant="ghost" size="lg" :data-open="open" @click="toggle"
            ...
            <div class="truncate">
                <div class="truncate">
                    ...
                    <span v-else class="text-sm group-hover:text-white/60 data-[open=true]:text-white/60">
                        Escolhe uma conta
                    </span>
                </div>
                <div v-if="selected"
                    class="text-right text-xs transition-colors group-hover:text-white/60 group:data-[open=true]:!text-white/60">
                    {{ selectedBalance }}
                </div>
            </div>
        ...
        <div v-if="open"
            ...
            <template v-if="props.accounts.length">
                <button v-for="a in props.accounts" :key="a.id" type="button" :class="[
                    ...
                    a.id === props.modelValue ? 'bg-primary' : 'bg-secondary text-white hover:text-night'
                ]" @click="select(a.id)">
                    <span class="truncate text-sm">{{ a.name }}</span>
                    <span class="text-xs shrink-0">{{ formatCentsEUR(currentBalanceById.get(a.id) ?? 0) }}</span>
                </button>
